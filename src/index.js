...
const ELEMENTS = ['calories','fat','cholesterol','sodium', 'carbs', 'fiber', 'sugar', 'protein', 'caffeine']
for(let i = 0; i < ELEMENTS.length; i++) {
  ...
  var drinkOne = { drink: 'Coffee', nutritions:[{'calories': 30 }, {'fat': 30 },{'cholesterol': 30 },{'sodium': 30 },{'carbs': 30 },{'fiber': 15 },{'sugar': 30 },{'protein': 30 },{'caffeine': 30 }]};
  var drinkTwo = { drink: 'Latte', nutritions:[{'calories': 90 },{'fat': 90 },{'cholesterol': 90 },{'sodium': 90 },{'carbs': 90 },{'fiber': 90 },{'sugar': 90 },{'protein': 90 },{'caffeine': 90 }] };
  ...
  var yScaleCalories = d3.scaleLinear()
    .domain([0, Math.max(Object.byString(drinkOne.nutritions[i], content), Object.byString(drinkTwo.nutritions[i], content))])
    .range([h-padding, padding]);
  
  var xAxisCalories = d3.axisBottom().scale(xScaleCalories);
  var yAxisCalories = d3.axisLeft().scale(yScaleCalories);
  
  var svgCalories = d3.select('#' + content)
    .append('svg')
    .attr('width', w)
    .attr('height', h);
  ...
  svgCalories.selectAll('rect')
    ...
    .attr('height', function(d) {
      return (yScaleCalories(0) - yScaleCalories(Object.byString(d.nutritions[i], content)));
    })
